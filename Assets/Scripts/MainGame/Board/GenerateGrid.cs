...
public class GenerateGrid
{
	...
	/// 
	///完成済みの数独盤から、空白マスを作りながら「解が一意になるように」問題を生成する処理 です。
	///全体の動き（流れ）
	/// 
	///1.全マスの座標を作る
	///(0,0) から (8,8) まで、81マスの座標を List<Vector2Int> に格納します。
	/// 
	///2.マスの順番をランダムにする（ShuffleNumbers）
	///空白にする候補マスをランダム順にして、どのマスを先に消すかを決めます。
	/// 
	///3.順番にマスを消していく
	///ランダム順に1マスずつ見て、まず値を退避（backup）し、そのマスを空（0）にします。
	/// 
	///4.一意解チェック
	///CountSolutions() を使って、現在の盤面が何通りの解を持つかを調べます。
	///この時、元の qGrid を壊さないように、Clone() や BlockCopy でコピーした作業用配列を渡します。
	/// 
	///5.結果による分岐
	///solutions == 1 → 唯一解なので、このマスは空白確定。
	///removed++ して、次のマスへ。
	///solutions > 1 → 複数解になるため、このマスは空けられない。
	///退避しておいた backup を戻します。
	/// 
	///6.空白数が目標に達したら終了
